...
namespace Symphony.Audio {
	 ...
	 * @param {System.ConfigType} config - System configuration data. See System.ConfigType for more details.
	 * @param {Function} eventListener - A callback that is passed to the MIDI.loadFile method.
	 * @return {void}
	 */
	export function playSong(song:any, MIDIJS:any, config:System.ConfigType, eventListener:Function = null):void {
		//Set the full path to the MIDI song that will be loaded.
		let midiFilePath = config.MIDI_DIRECTORY + (_.endsWith(config.MIDI_DIRECTORY, '/') ? '' : '/') + song.filename;
		...
		MIDIJS.Player.loadFile(midiFilePath, (data) => {
	 ...
	 * @return {void}
	 */
	export function stopSong(midiPlayer):void {
		//Check to see if music is already playing. If so, stop the music.
